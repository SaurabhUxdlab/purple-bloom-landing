        ...
        <div className="hidden md:flex items-center justify-center gap-10 justify-self-center">
          {links.map((l) => (
            <a
              ...
              className="font-ui text-[15px] lg:text-[17px] font-medium text-white hover:text-white transition-colors"
            >
              {l}
            </a>
          ))}
        ...
        <div className="md:hidden bg-[#3D0642] backdrop-blur-md border-t border-white/10 p-4 space-y-3">
          {links.map((l) => (
            <a
              key={l}
              href={l === "Home" ? "/" : `${l.toLowerCase()}`}
              className="font-ui block py-2 text-base font-medium text-white hover:text-white"
